...
#ifndef __SOF_LIB_GNA2_TLV_READER_H__
#define __SOF_LIB_GNA2_TLV_READER_H__
 ...
 */
GNA2_TLV_LINKAGE Gna2TlvStatus Gna2TlvCheckValid(const Gna2TlvRecord *tlvRecord,
						 const char *tlvBlobEnd)
{
 ...
 * Can be used in loop to eventually find every occurrences of the record with the
 * specified type if there is more than one within the blob (tlvArrayBegin and
 * tlvArraySize have to be modified accordingly).
 *
 * @param [in] tlvArrayBegin Address of the TLV record to start the search. TLV
 * formatted GNA model address.
 * @param [in] tlvArraySize Byte size of all the TLV records list in the memory
 ...
 */
GNA2_TLV_LINKAGE Gna2TlvStatus Gna2TlvFindInArray(const char *tlvArrayBegin, uint32_t tlvArraySize,
						  const Gna2TlvType tlvTypeToFind,
						  uint32_t *outValueLength, void **outValue)
{
 ...
 *  - the provided memory region - [tlvArrayBegin, tlvArrayBegin+tlvArraySize)
 * consists of a list of complete TLV records. In particular, no record is
 * truncated with the end of the region.
 *
 * Can be called before reading from TLV blob in memory.
 *
 * @param [in] tlvArrayBegin Address of the TLV record to start the verification.
 * e.g., TLV formatted GNA model address.
 * @param [in] tlvArraySize Byte size of all the TLV records list in the memory
 * which should be verified.
 * @return Status of the operation.
 * @retval Gna2TlvStatusSuccess Successful verification of version and
 * completeness.
 ...
 * @retval Gna2TlvStatusSecondaryVersionFound The blob contains more than one
 * record with TLV version.
 * @retval Gna2TlvStatusTlvReadError Record is truncated with the end of the
 * region.
 */
GNA2_TLV_LINKAGE Gna2TlvStatus Gna2TlvVerifyVersionAndCohesion(const char *tlvArrayBegin,
							       uint32_t tlvArraySize)
{
	void *version = NULL;
	uint32_t length = 0;
	Gna2TlvStatus status = Gna2TlvFindInArray(tlvArrayBegin, tlvArraySize,
						  Gna2TlvTypeTlvVersion, &length, &version);
	...
	status = Gna2TlvFindInArray(tlvArrayBegin, tlvArraySize, Gna2TlvTypeTlvVersion, &length,
				    &version);
...
#endif /* __SOF_LIB_GNA2_TLV_READER_H__ */
